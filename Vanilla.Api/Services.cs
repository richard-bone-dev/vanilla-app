...
namespace Vanilla.Api.Services;
...
public sealed class LedgerApplicationService(AppDbContext dbContext, IClock clock)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        return EntryMutationResult.Success(
            createdItem,
            new CustomerSummaryResponse(customer.Id, customer.Name, resultingBalance),
            resultingBalance,
            entryType == "Payment" && resultingBalance == 0m);
    ...
    private async Task<decimal> GetCustomerBalanceAsync(Guid customerId, CancellationToken cancellationToken)
    {
        var orderTotal = await _dbContext.Orders
            .Where(order => order.CustomerId == customerId)
            .SumAsync(order => (decimal?)order.Amount, cancellationToken) ?? 0m;
    ...
    private static CreatedItemResponse BuildCreatedItem(Order order) =>
        new(order.Id, "Order", order.CustomerId, order.Amount, order.Notes, order.CreatedUtc);

    private static CreatedItemResponse BuildCreatedItem(Payment payment) =>
        new(payment.Id, "Payment", payment.CustomerId, payment.Amount, payment.Notes, payment.CreatedUtc);
}

public sealed record EntryMutationResult(
    CreatedItemResponse? CreatedItem,
...
{
    public static EntryMutationResult Success(
        CreatedItemResponse createdItem,
        CustomerSummaryResponse customer,
        decimal resultingBalance,
        bool requiresSettlementConfirmation) =>
    ...
    public static EntryMutationResult Validation(string key, string message) =>
        new(null, null, null, false, false, new Dictionary<string, string[]>
        {
            [key] = [message]
        });
}
...
public sealed record QuickEntryResult(
    QuickEntryResponse? Response,
    bool IsNotFound,
    IReadOnlyDictionary<string, string[]>? ValidationErrors)
{
    public static QuickEntryResult Success(QuickEntryResponse response) => new(response, false, null);

    public static QuickEntryResult RequiresCustomerConfirmation(string customerName) =>
        new(
            new QuickEntryResponse(null, null, null, true, false, $"Customer '{customerName}' was not found."),
            false,
            null);

    public static QuickEntryResult NotFound(string message) =>
        new(new QuickEntryResponse(null, null, null, false, false, message), true, null);

    public static QuickEntryResult Validation(string key, string message) =>
