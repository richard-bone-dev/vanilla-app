...
public sealed class ConfigurationAndEncryptionTests
{
    ...
    public void Connection_string_can_be_read_from_config()
    {
        ...
        AppConfiguration.GetRequiredConnectionString(configuration, "SqlServer")
            .Should().Be("Server=sqlserver,1433;Database=VanillaApp;");
    }
    ...
    public async Task Customer_phone_encrypts_and_decrypts()
    {
        ...
        var customer = await dbContext.Customers.SingleAsync();
        customer.Phone.Should().Be("12345");
    }
    ...
    public async Task Customer_notes_encrypt_and_decrypt()
    {
        ...
        var customer = await dbContext.Customers.SingleAsync();
        customer.Notes.Should().Be("customer note");
    }
    ...
    public async Task Order_notes_encrypt_and_decrypt()
    {
        ...
        order.Notes.Should().Be("order note");
    }
    ...
    public async Task Amount_remains_queryable_as_decimal()
    {
        ...
        dbContext.Orders.AddRange(
            new Order { Customer = customer, Amount = 12.50m, CreatedUtc = DateTime.UtcNow },
            new Order { Customer = customer, Amount = 7.50m, CreatedUtc = DateTime.UtcNow });

        await dbContext.SaveChangesAsync();

        var sum = await dbContext.Orders.SumAsync(order => order.Amount);
