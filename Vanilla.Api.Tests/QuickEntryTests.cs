...
public sealed class QuickEntryTests
{
    ...
    public async Task Payment_can_create_negative_balance()
    {
        ...
        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var payload = await response.Content.ReadFromJsonAsync<QuickEntryResponse>();
        payload!.ResultingBalance.Should().Be(-10m);
    }
    ...
    public async Task Unknown_customer_with_auto_create_customer_if_missing_false_returns_requires_customer_confirmation()
    {
        ...
        var response = await client.PostAsJsonAsync("/api/quick-entry", new QuickEntryRequest("Order", null, "Missing Customer", 12m, null, false));

        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var payload = await response.Content.ReadFromJsonAsync<QuickEntryResponse>();
        payload!.RequiresCustomerConfirmation.Should().BeTrue();
    }
    ...
    public async Task Unknown_customer_with_auto_create_customer_if_missing_true_creates_customer_and_entry()
    {
        ...
        var response = await client.PostAsJsonAsync("/api/quick-entry", new QuickEntryRequest("Order", null, "New Customer", 12m, null, true));
    ...
    public async Task Missing_customer_fails_validation()
    {
        ...
        var response = await client.PostAsJsonAsync("/api/quick-entry", new QuickEntryRequest("Order", null, null, 10m, null, false));
    ...
    public async Task Zero_or_negative_amount_fails_validation(decimal amount)
    {
        ...
        var response = await client.PostAsJsonAsync("/api/quick-entry", new QuickEntryRequest("Payment", customerId, null, amount, null, false));
