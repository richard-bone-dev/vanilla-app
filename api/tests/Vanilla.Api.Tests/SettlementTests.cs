...
public sealed class SettlementTests
{
    ...
    public async Task Settlement_fails_if_active_balance_is_not_zero()
    {
        ...
        (await factory.CreateClient().PostAsync($"/api/customers/{customerId}/settle", null)).StatusCode.Should().Be(HttpStatusCode.BadRequest);
    }
    ...
    public async Task Settlement_succeeds_if_active_balance_is_zero_soft_deletes_everything_and_returns_row_counts()
    {
        ...
        var response = await factory.CreateClient().PostAsync($"/api/customers/{customerId}/settle", null);
    ...
    public async Task Settled_records_are_excluded_from_search_ledger_and_dashboard_summary()
    {
        ...
        (await client.GetFromJsonAsync<List<CustomerSearchItem>>("/api/customers/search?query=Ali")).Should().BeEmpty();
        (await client.GetAsync($"/api/customers/{customerId}/ledger")).StatusCode.Should().Be(HttpStatusCode.NotFound);
